...
fn monitored_work(worker_id: usize, task_id: usize) -> Result<usize, String> {
    ...
    tracing::debug!(worker = worker_id, task = task_id, "Monitored task execution progress");
    Ok(task_id)
}
...
fn test_logger_under_heavy_concurrency() {
        ...
        for t_id in 0..num_threads {
            let handle = thread::spawn(move || {
                for task_id in 0..num_tasks_per_thread {
                    // 模拟同步上下文追踪
                    let _ctx = owl_logger::context::with_request_id(&format!("req-{}-{}", t_id, task_id));
                    ...
                    // 混合常规并发日志
                    if task_id % 10 == 0 {
                        tracing::info!(worker = t_id, task = task_id, "Normal concurrency log item");
                    }
                }
            });
            handles.push(handle);
    ...
    assert!(error_log_path.exists(), "Error separation log file must be created");
    ...
    for line in &main_lines {
        let val: serde_json::Value = serde_json::from_str(line)
            .expect("Each line in main log must be valid JSON");
    ...
    for line in &error_lines {
        let val: serde_json::Value = serde_json::from_str(line)
            .expect("Each line in error log must be valid JSON");
    ...
    assert!(error_lines.len() > 0, "Should capture worker error logs");
